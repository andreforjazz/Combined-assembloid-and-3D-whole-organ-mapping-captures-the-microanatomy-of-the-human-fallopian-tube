function deeplab_classification(pth5x,pthDL,sxy,nm,cmap,nblack,nwhite,outpth)
...
if size(cmap,2)==3
    ...
    outpthcolor=[outpth,'color\'];mkdir(outpthcolor);
end
...
%                 subplot(1,2,2),imshow(imcolor)
    ...
    if cc==1
        ...
        imwrite(imcolor,[outpthcolor,imlist(kk).name(1:end-3),'jpg']);
        if kk==1
            figure(18);subplot(1,2,1),imshow(uint8(im));subplot(1,2,2),imshow(imcolor);
            ha=get(gcf,'Children');linkaxes(ha);pause(0.5)
        end
    end
