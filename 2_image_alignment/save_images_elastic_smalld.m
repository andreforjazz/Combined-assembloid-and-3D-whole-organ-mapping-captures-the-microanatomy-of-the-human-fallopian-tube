...
function [rot,rr]=get_cropim(pthdata,scale)
    ...
    z1=round(length(imlist)/3);
    z2=round(length(imlist)/2);
    z3=round(length(imlist)*2/3);
    
    im1=rgb2gray(imread([pth1,imlist(z1).name]));
    im2=rgb2gray(imread([pth1,imlist(z2).name]));
    im3=rgb2gray(imread([pth1,imlist(z3).name]));
